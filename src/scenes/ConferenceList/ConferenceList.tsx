...
const ConferenceListPage: React.FC<Props> = ({
  ...
  const [hitsPerPage, setHitsPerPage] = useState(200)
  ...
  const updateUrlQueryParams = (algoliaSearchState: SearchState) => {
    ...
    console.log(algoliaSearchState)
  ...
  return (
    <Page
      ...
      title='Find your next tech conference in the Polkadot ecosystem'
      subtitle='Open-source and crowd-sourced list of meetup around pulkadot ecosystem'
    >
      <InstantSearch
      ...
      >
        ...
        <p className={styles.HeaderLinks}>
          <Link url='#'>General</Link>
          <Link url='#'>Meetup</Link>
          <Link url='#'>Discussion</Link>
          <Link url='#'>Event</Link>
          <Link url='#'>Learning</Link>          
          <Link url='/conferences/new'>Add conference</Link>
        </p>
